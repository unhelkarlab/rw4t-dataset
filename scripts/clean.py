...
def clean_trajectory(states, actions, rewards, sectasks, step_penalty=0.0):
    states, actions = transition_robot_pause(states, actions)
    states, actions = transition_robot_success(states, actions)
    _, _, sectasks = correct_simultaneous_moves(states,
                                                actions,
                                                sectasks,
                                                step_penalty=step_penalty)
    states, actions, rewards = correct_simultaneous_moves(
        states, actions, rewards, step_penalty=step_penalty)
    states, actions, rewards, sectasks = remove_waits(states, actions, rewards,
                                                      sectasks)
    return states, actions, rewards, sectasks


def correct_simultaneous_moves(states, actions, rewards, step_penalty=0.0):
    '''
    Separates simultaneous movements

    Simultaneous movements happen when the player moved himself
    ...
    for idx in idxs:
        ...
        rewards = np.insert(rewards, idx, step_penalty)
...
def remove_waits(states, actions: np.array, rewards, sectasks):
    ...
    keep_idxs = np.concatenate([
        active_idx, wait_idx, robot_picks_idx, robot_transition_idxs,
        positive_rewards
    ])
    keep_idxs = sorted(np.unique(keep_idxs))
    return states[keep_idxs], actions[keep_idxs], rewards[keep_idxs], sectasks[
        keep_idxs]
...
def _consecutive_robot_usage(actions):
    ...
    for idx in range(len(active_actions) - 1):
        if 'toObj' in str(active_actions[idx]) and 'toObj' in str(
                active_actions[idx + 1]):
