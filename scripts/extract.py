...
def get_trajs(num_bins=None):
    ...
    for user in os.listdir(DATA_FOLDER):
        for trial in range(3, 8):
            st, acts, rews, dones, sectaskbool = get_trial_behavior(user, trial, num_bins)
    ...
    return mdp_states, mdp_actions, mdp_rewards, mdp_dones, ids, sec_task_present
...
np.save(os.path.join(TRAJ_DIR,"discrete\\states2"), mdp_states, allow_pickle=False)
np.save(os.path.join(TRAJ_DIR,"discrete\\actions2"), mdp_actions)
np.save(os.path.join(TRAJ_DIR,"discrete\\rewards2"), mdp_rewards, allow_pickle=False)
np.save(os.path.join(TRAJ_DIR,"discrete\\dones2"), mdp_dones, allow_pickle=False)
np.save(os.path.join(TRAJ_DIR,"discrete\\ids2"), ids, allow_pickle=False)
np.save(os.path.join(TRAJ_DIR,"discrete\\sectasks2"), sectasks, allow_pickle=False)
